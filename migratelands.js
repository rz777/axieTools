...
const __CONTRACT_ITEM = '0x8c811e3c958e190f5ec15fb376533a3398620500';
...
async function migrateLands() {
    ...
    var graph = `https://axieinfinity.com/graphql-server-v2/graphql?query={lands(from:0,size:100,sort:IdAsc,owner:"${wallet.address}"){results{tokenId,auction{listingIndex}}}}`;
    var json = (await fetch(graph).then(response=>response.json()));
    var lands = json.data.lands.results;
    var contract = new ethers.Contract(__CONTRACT_ITEM, __ABI_ITEM, wallet);
    // start
    for (var i=0; i<lands.length; i++) {
        if (lands[i].auction===null) {
            try {
                ...
                console.log(`ITEM: ${lands[i].tokenId} -> TO: ${to} -> ${res.hash}`);
